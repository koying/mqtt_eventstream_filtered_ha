...
DOMAIN = "mqtt_eventstream_filtered"
CONF_PUBLISH_TOPIC = "publish_topic"
CONF_SUBSCRIBE_TOPIC = "subscribe_topic"
CONF_PUBLISH_EVENTSTREAM_RECEIVED = "publish_eventstream_received"
CONF_INCLUDE_EVENT = "include_event"
CONF_EXCLUDE_EVENT = "exclude_event"
        ...
        DOMAIN: vol.Schema(
            {
                ...
                vol.Optional(CONF_INCLUDE_EVENT, default=[]): cv.ensure_list,
                vol.Optional(CONF_EXCLUDE_EVENT, default=[]): cv.ensure_list,
            }
        )
    },
...
async def async_setup(hass, config):
    ...
    include_event = conf.get(CONF_INCLUDE_EVENT)
    excude_event = conf.get(CONF_EXCLUDE_EVENT)
    excude_event.append(EVENT_TIME_CHANGED)
    ...
    async def _event_publisher(event):
        ...
        if include_event:
            if event.event_type not in include_event:
                return
        elif event.event_type in excude_event:
            return
